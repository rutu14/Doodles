import { Box, Button, Center, CircularProgress, Flex, Grid, GridItem, Heading, Popover, PopoverArrow,PopoverBody, PopoverCloseButton, PopoverContent, PopoverHeader, PopoverTrigger, useToast } from "@chakra-ui/react";
import { useEffect } from "react";
import { useDispatch, useSelector } from "react-redux";
import { useNavigate } from "react-router";
import { FilterFunnelIcon, SortIcon } from "../../assest/icon";
import { getAllNotes, noteSelector, getTags, tagSelector } from "../../redux";
...
const Dashboard = () => {
    ...
    const notePage = (noteDetail) => navigate('/view',{state:{noteDetail}});
            ...
            :
                ...
                : notes && notes.map((noteDetail) => {
                    ...
                    <Box key={noteDetail._id} as={Grid} w={'100%'} h={'68px'} alignItems={'center'} gridTemplateColumns={'40%  20% 20% 20%'} gap={'20px'} m={'10px 0'} bg={noteDetail.color !== 'default' ?`${noteDetail.color}.100`:'#ccccff75'} fontSize={'14px'} lineHeight={'33px'} letterSpacing={'.0892857143em'} textTransform={'uppercase'} p={'0 8px'} color={'#808080'} boxShadow={"0 2px 1px -1px rgb(0 0 0 / 20%), 0 1px 1px 0 rgb(0 0 0 / 14%), 0 1px 3px 0 rgb(0 0 0 / 12%)"} borderRadius={'4px'} cursor={'pointer'} >                    
                        <GridItem p={'16px'} overflowX={'clip'} whiteSpace={'nowrap'} fontWeight={'medium'} fontSize={'20px'} lineHeight={'32px'} _hover={{color:'#070707', textDecoration:'none'}} onClick={()=>notePage(noteDetail)}>
                            {noteDetail.title}
                        </GridItem>
                        { noteDetail.tags ?
                            <Popover>
